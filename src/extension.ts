import { ALL_RULE_NAMES, LitAnalyzerConfig } from "wapitis-analyzer";
import { join } from "path";
import { ColorProvider } from "./color-provider";
import * as vscode from "vscode";

const tsLitPluginId = "ts-wapitis-plugin";
const typeScriptExtensionId = "vscode.typescript-language-features";
const configurationSection = "wapitis-plugin";
const configurationExperimentalHtmlSection = "html.experimental";
const analyzeCommandId = "wapitis-plugin.analyze";
...
function getConfig(): Partial<LitAnalyzerConfig> {
	...
	// ts-wapitis-plugin will make sure that deprecated rules are mapped correctly to new rules
	[
		"skipSuggestions",
		"checkUnknownEvents",
		"skipUnknownTags",
	...
	withConfigValue(experimental, "customData", value => {
		// Merge value from vscode with "wapitis-plugin.customHtmlData"
...
function handleAnalyzeCommand() {
	vscode.window
		...
		.then((glob: string | undefined) => {
			...
			const cliCommand = `npx wapitis-analizer "${glob}"`;
			const terminal = vscode.window.createTerminal("wapitis-analizer");
